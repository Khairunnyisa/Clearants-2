...
const CardBackgroundPath = ({
  ...
  useLayoutEffect(() => {
    ...
    const updateDimensions = () => {
      if (containerRef.current) {
        ...
        setDimensions({ width, height, vw: window.innerWidth / 100 });
      }
    };
  ...
  const parseValue = useCallback((val) => {
    if (typeof val === "string" && val.endsWith("vw")) return parseFloat(val) * dimensions.vw;
  ...
  const d = useMemo(() => {
    ...
    return `
      ...
      A ${r} ${r} 0 0 1 ${r} 0
      H ${ts}
      C ${ts + tw / 3} 0, ${ts + (tw * 2) / 3} ${heightCorner}, ${ts + tw} ${heightCorner}
      H ${w - r}
